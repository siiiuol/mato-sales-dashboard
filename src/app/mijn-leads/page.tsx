...
import { LEAD_STATUSES, categoryLabel, statusLabel } from "@/lib/constants";
      ...
      {leads.length === 0 ? (
        <section className="panel p-8 text-center space-y-3">
          ...
          <p className="text-sm text-[var(--text-dim)]">
            Zoek in de lijst een zaak die je wil bellen en druk op{" "}
...
type Row = {
  ...
  outreach: Array<{ outcome: string | null; createdAt: Date; note: string | null }>;
              ...
              <tr key={lead.id}>
                ...
                <td>
                  ...
                  <div className="text-xs text-[var(--text-dim)]">
                    {[lead.city, categoryLabel(lead.category)]
                      .filter(Boolean)
                      .join(" · ")}
                  </div>
                  {overdue && (
                ...
                <td className="mono text-xs">
                  {lead.phone ? (
                    <a href={`tel:${lead.phone}`} className="text-[var(--accent)]">
                      {lead.phone}
                    </a>
                  ) : (
                ...
                <td className="text-xs text-[var(--text-dim)]">
                  {last
                    ? `${last.createdAt.toLocaleDateString("nl-BE")} · ${last.outcome ?? "genoteerd"}`
                    : "nog niet gebeld"}
                </td>
                <td>
